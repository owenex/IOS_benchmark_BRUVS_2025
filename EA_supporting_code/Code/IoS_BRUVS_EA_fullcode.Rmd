...
species_obs <- read.csv('../Data/IoS_BEN_observations.csv')
## just demersal species
reef_fish <- read.csv('../Data/IoS_BEN_observations_demerselfish.csv')
## read in lengths data
species_lengths <- read.csv('../Data/IoS_BEN_Lengths.csv')

## read in processed biomass data
site_biomass <- read.csv('../Data/IOS_BEN_biomass.csv')
## read in metadata
envo_variables <- read.csv('../Data/IoS_BEN_metadata.csv')
...
deployment_richness <- reef_fish %>%
  group_by(GRTS_ID) %>%
  summarise(Richness = n_distinct(Species))  # more accurate than length()

# Calculate mean MaxN per site
deployment_maxn <- reef_fish %>%
  group_by(GRTS_ID) %>%
  summarise(Mean_MaxN = mean(MaxN, na.rm = TRUE))

# Merge richness and mean MaxN
mean_values1 <- left_join(deployment_maxn, deployment_richness, by = "GRTS_ID")

# Calculate total MaxN per site
...
# Calculate Shannon's diversity per site
shannon_diversity <- species_obs %>%
  group_by(GRTS_ID) %>%
  summarise(shannon_index = diversity(MaxN, index = "shannon")) 

# Merge diversity data
mean_values3 <- left_join(mean_values2, shannon_diversity, by = "GRTS_ID")

# --- Mean Length for Benthic-Associated Species ---
# Subset species lengths to those observed in demersal (reef_fish) 
benthic_species <- unique(reef_fish$Species)
species_lengths_filtered <- species_lengths %>%
  filter(Species %in% benthic_species)

# Calculate mean length per site (convert mm to cm)
deployment_length <- species_lengths_filtered %>%
  group_by(GRTS_ID) %>%
  summarise(Mean_length = mean(Length, na.rm = TRUE) / 10)

# Merge mean length
mean_values4 <- left_join(mean_values3, deployment_length, by = "GRTS_ID")

# --- Add Total Biomass ---
site_biomass_clean <- site_biomass %>%
  select(GRTS_ID, Total_biomass_kg)

mean_values5 <- left_join(mean_values4, site_biomass_clean, by = "GRTS_ID")

# --- Join Environmental Variables ---
# Drop unnecessary columns and duplicates
envo_variables_clean <- envo_variables %>%
  select(2:27) %>%
  distinct()

# Subset columns of interest
envo_variables_short <- envo_variables_clean %>%
  select(GRTS_ID, Date, Broadscale_habitat, Habitat, Mixed, Relief_score,
         Lat, Lon, TimeIN, Depth, Temp, Rug_range, Remoteness,
         Exposure, Slope_mean, MCZ, SAC, Height_of_tide) %>%
  mutate(Habitat = factor(Habitat),
    Broadscale_habitat = factor(Broadscale_habitat),
    Relief_score = factor(Relief_score))

# Merge environmental variables with response data
IOS_data <- left_join(mean_values5, envo_variables_short, by = "GRTS_ID")

# Assign Year and Month
IOS_data <- IOS_data %>%
  mutate(Year = substr(Date, 7, 10),Month = substr(Date, 4, 5))

# Optional: also mutate `envo_variables_short` if needed elsewhere
# envo_variables_short <- envo_variables_short %>%
#   mutate(Year = substr(Date, 7, 10))

# Filter for SAC sites only
IOS_data_SAC <- IOS_data %>%
  filter(SAC > 0)
...
```{r, results='hide'}
# --- Summary Statistics & Dispersion Indices ---
## Richness
...
# --- Pearson Correlation Matrix & Multicollinearity Checks (VIF) ---
## Select numeric environmental variables (first pass)
envo_variables_numeric <- IOS_data[,c('Depth','Temp','Remoteness','Exposure','Relief_score','Slope_mean','Rug_range')]
envo_variables_numeric <-mutate_all(envo_variables_numeric, function(x)as.numeric(as.character(x)))

## Pearson correlation matrix
correlation_matrix1 <- cor(envo_variables_numeric)
print(correlation_matrix1)

## Setting up the model
Richness_model <- lm(Richness ~ Relief_score+Depth+Temp+Rug_range+Remoteness+Exposure+Slope_mean, data = IOS_data)
MaxN_model <- lm(Total_MaxN ~ Relief_score+Depth+Temp+Rug_range+Remoteness+Exposure+Slope_mean, data = IOS_data)
Div_model <- lm(shannon_index ~ Relief_score+Depth+Temp+Rug_range+Remoteness+Exposure+Slope_mean, data = IOS_data)
Biomass_model <- lm(Total_biomass_kg ~ Relief_score+Depth+Temp+Rug_range+Remoteness+Exposure+Slope_mean, data = IOS_data)

### Calculating VIF 
vif_Richness <- as.data.frame(vif(Richness_model))
vif_Mean_MaxN <- as.data.frame(vif(MaxN_model))
vif_Div <- as.data.frame(vif(Div_model))
vif_Biomass_index <- as.data.frame(vif(Biomass_model))

## merge together
vif_all <- vif_Richness %>%
  cbind(vif_Mean_MaxN) %>%
  cbind(vif_Div) %>%
  cbind(vif_Biomass_index)
vif_all

# --- Rerun Analysis Omitting Correlated Predictors
## subset variables of interest for pearson
envo_variables_numeric <- IOS_data[,c('Depth','Temp','Remoteness','Exposure','Relief_score')]
envo_variables_numeric <-mutate_all(envo_variables_numeric, function(x)as.numeric(as.character(x)))

## Pearson correlation matrix
correlation_matrix2 <- cor(envo_variables_numeric)
print(correlation_matrix2)

## Setting up the model
Richness_model2 <- lm(Richness ~ Relief_score+Depth+Temp+Remoteness+Exposure, data = IOS_data)
MaxN_model2 <- lm(Total_MaxN ~ Relief_score+Depth+Temp+Remoteness+Exposure, data = IOS_data)
Biomass_model2 <- lm(Total_biomass_kg ~ Relief_score+Depth+Temp+Remoteness+Exposure, data = IOS_data)

## Calculating VIF 
vif_Richness2 <- as.data.frame(vif(Richness_model2))
vif_Mean_MaxN2 <- as.data.frame(vif(MaxN_model2))
vif_Biomass_index2 <- as.data.frame(vif(Biomass_model2))

## merge together
vif_all2 <- vif_Richness2 %>%
  cbind(vif_Mean_MaxN2) %>%
  cbind(vif_Biomass_index2)
vif_all2
...
```{r, warning=FALSE}
# Read and Format Observation Data ---
## Read wide-format observation data with zero values
species <- read.csv('../Data/iNEXT_data_format2.csv')

## Convert from wide to long format
long <- melt(setDT(species), id.vars = "ID", variable.name = "Species")
colnames(long) <- c('GRTS_ID', 'Species', 'MaxN')
long$GRTS_ID <- as.integer(gsub("IOS_", "", as.character(long$GRTS_ID)))
